...
module warp #(
    ...
    logic [15:0] instr_muxed;         // Instruction after bubble insertion
    ...
    logic [15:0] bimm_exe, rs_data_exe, rt_data_exe;
    ...
    logic [15:0][15:0] mem_data_reg, rd_data_exe2;
    logic        regwe_exe2, sel_imm_exe2, set_nzp_exe2;
    ...
    assign stalled_on_alu    = (alu_req_exe_true) & !alu_access; //Does this logic work
    ...
    assign alu_req_de_true   = (alu_req_de & (mask_de != 16'hFFFF) & !stalled_on_mem); //played with this too
    ...
    always_ff @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            halt_prev <= halt;
            if (!halt_prev & halt) instr_missed <= instr_in;
        end
    end
    assign instr = (halt_prev & !halt) ? instr_missed : instr_in;
    ...
    always_ff @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            pc <= pc - 1 + {{2{bimm_de[7]}}, bimm_de[7:0]};
        end else begin
            pc <= pc_next;
        end
    end
...
// --- Data Forwarding (in Decode) ---
    ...
    // --- Writeback Mux ---
    assign rd_data_exe2 = (mem_ldr_we_exe2) ? mem_to_reg_data :
    ...
    // Speculative flags from EXE stage
    /*logic [15:0][2:0] nzp_flags_speculative;
    always_comb begin
        for (int i = 0; i < 16; i++) begin
            if (exe_out[i][15])           nzp_flags_speculative[i] = 3'b100;
            else if (exe_out[i] == 16'd0) nzp_flags_speculative[i] = 3'b010;
            else                          nzp_flags_speculative[i] = 3'b001;
        end
    end*/
    
    // Updated forwarding two level forwarding(add EXE level)
    always_comb begin
        //if (set_nzp_exe && !halt)
        //    nzp_flags_true = nzp_flags_speculative;
        //else 
        if (set_nzp_exe2)
