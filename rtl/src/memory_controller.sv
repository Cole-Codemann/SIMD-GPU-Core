...
module memory_controller (
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            queue_count <= queue_count
                + 3'((warp_waiting[warp_id[1:0]] == 1'b0) & (|store_requests | |load_requests))
                - 3'((state == STORE_FINISH || state == LOAD_FINISH));
        end
    end
    ...
    always_ff @(posedge clk) begin
        reset_prev <= rst;
        if (reset_prev && !rst) mem_controller_usage <= '0;
        else if (~rst) begin
            if (queue_count != '0) mem_controller_usage <= mem_controller_usage + 1'd1;
        end
    end        
