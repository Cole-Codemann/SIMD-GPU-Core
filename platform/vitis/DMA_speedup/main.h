 ...
 * MicroBlaze broadcasts each write to all 4 warp banks simultaneously.
 *
 ...
 *
 * Channel 1 (offset 0x00): output — control register
 *   Bit [0] — GPU reset (active-high)
 *   Bit [1] — Session counter reset (active-high)
 *   Bit [2] — DMEM address space select
 *              0 = GPU sees DMEM workspace A (lower half)
 *              1 = GPU sees DMEM workspace B (upper half)
 *   Bit [3] — IMEM address space select
 *              0 = GPU sees IMEM bank A (lower half)
 *              1 = GPU sees IMEM bank B (upper half)
 *
 * Channel 2 (offset 0x08): input — GPU_done back to MicroBlaze.
 *   Bit [0] = 1 when all warps have reached DONE.
 * ============================================================ */
...
#define GPU_RESET_BIT           (1u << 0)
#define SESSION_CLK_RESET_BIT   (1u << 1)
#define DMEM_SPACE_BIT          (1u << 2)
#define IMEM_SPACE_BIT          (1u << 3)

/* Composite reset states (address space bits OR'd in separately) */
#define GPU_HOLD_CLK_HOLD       (GPU_RESET_BIT | SESSION_CLK_RESET_BIT)
#define GPU_HOLD_CLK_RUN        (GPU_RESET_BIT)
#define GPU_RUN_CLK_HOLD        (SESSION_CLK_RESET_BIT)
#define GPU_RUN_CLK_RUN         (0x0u)

/* Current address space — module-level state so every GPIO write
 * preserves the selected workspace without the caller tracking it. */
static u32 addr_space_bits = 0;

/* ---- address space selection ---- */
static inline void set_dmem_space(u32 space) {
    if (space) addr_space_bits |= DMEM_SPACE_BIT;
    else       addr_space_bits &= ~DMEM_SPACE_BIT;
}

static inline void set_imem_space(u32 space) {
    if (space) addr_space_bits |= IMEM_SPACE_BIT;
    else       addr_space_bits &= ~IMEM_SPACE_BIT;
}

static inline void set_workspace(u32 dmem_space, u32 imem_space) {
    addr_space_bits = (dmem_space ? DMEM_SPACE_BIT : 0)
                    | (imem_space ? IMEM_SPACE_BIT : 0);
}

static inline u32 dmem_space(void) { return (addr_space_bits >> 2) & 1; }
static inline u32 imem_space(void) { return (addr_space_bits >> 3) & 1; }

/* ---- GPU control (preserves address space bits) ---- */
static inline void gpu_hold(void)        { Xil_Out32(GPIO_CTRL_OUT, GPU_HOLD_CLK_HOLD | addr_space_bits); }
static inline void gpu_release_run(void) { Xil_Out32(GPIO_CTRL_OUT, GPU_RUN_CLK_RUN   | addr_space_bits); }
static inline void gpu_stop(void)        { Xil_Out32(GPIO_CTRL_OUT, GPU_HOLD_CLK_HOLD | addr_space_bits); }
static inline u32  gpu_done(void)        { return Xil_In32(GPIO_DONE_IN) & 0x1u; }

static inline void gpu_hold_keep_timer(void) {
    Xil_Out32(GPIO_CTRL_OUT, GPU_HOLD_CLK_RUN | addr_space_bits);
}
